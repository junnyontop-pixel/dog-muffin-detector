    ...
    for i, img in enumerate(images):
        ...
        try:
            ...
            if img_url and 'http' in img_url:
                # 💡 팁: 현재 폴더의 파일 개수를 세서 그 다음 번호부터 저장하게 함
                current_files = len(os.listdir(save_path)) 
                file_name = f"{current_files}.jpg"
                
                urllib.request.urlretrieve(img_url, os.path.join(save_path, file_name))
                downloaded += 1
                if downloaded % 10 == 0:
...
if __name__ == "__main__":
    ...
    collect_images("chihuahua eyes and nose", "data/dog", 50)
    collect_images("muffin in paper cup", "data/muffin", 50)